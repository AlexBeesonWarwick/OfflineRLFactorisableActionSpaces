...
class AtomicDQN(nn.Module):
    def __init__(self, state_dim, action_dim, num_critics=2, hidden_dim=512):
        super(AtomicDQN, self).__init__()

        self.l1 = VectorizedLinear(state_dim, hidden_dim, num_critics)
        self.l2 = VectorizedLinear(hidden_dim, hidden_dim, num_critics)
        self.qs = VectorizedLinear(hidden_dim, action_dim, num_critics)
    ...
    def forward(self, state):
        ...
        return q_values.reshape(self.num_critics, -1, self.action_dim)


class Agent():
    def __init__(self, state_dim, action_dim, num_critics=2,
                 batch_size=256, lr=3e-4, gamma=0.99, tau=0.005, device="cpu"):

        # Initialisation
        self.dqn = AtomicDQN(state_dim, action_dim, num_critics).to(device)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            q_vals = self.dqn(state).mean(0)
            action = torch.argmax(q_vals, -1)

        return action.item()

    def train(self, replay_buffer, iterations=1, alpha_prime=0.5):
        ...
        for it in range(iterations):
            ...
            with torch.no_grad():
                next_q_vals = self.dqn_target(next_state)
                next_q_vals_mean = next_q_vals.mean(0)
                next_q_vals_max = next_q_vals_mean.max(-1)[0]
                q_hat = reward + self.gamma * (1 - done) * next_q_vals_max
            q_vals = self.dqn(state)
            actions = action.long().reshape(self.batch_size, -1)
            q_vals_action = torch.zeros(self.num_critics, self.batch_size).to(self.device)
            for k in range(self.num_critics):
                q_vals_action[k] = q_vals[k].gather(1, actions).squeeze(-1)
            dqn_loss = F.huber_loss(q_vals_action, q_hat)  # Ignore warning - we want broadcasting

            # CQL
            q_vals_mean = q_vals.mean(0)
            logsumexp = torch.logsumexp(q_vals_mean, -1)
            q_vals_action_mean = q_vals_action.mean(0)
            p = (logsumexp - q_vals_action_mean).mean()
