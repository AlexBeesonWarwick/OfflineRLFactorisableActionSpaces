...
class Agent():
    ...
    def choose_action(self, state, lmbda):
    ...
    def train(self, replay_buffer, iterations=1):
        ...
        for it in range(iterations):
            ...
            ## DecQN ##
            with torch.no_grad():
                next_action_probs = self.policy_bc(next_state).exp()
                next_u_vals = self.dqn_target(next_state).mean(0)
                vs = (next_u_vals * next_action_probs).sum(-1)
                target_vf_pred = vs.mean(-1)
